...
((function(...args) {
  ...
  var group = behavior => (obj, value, context) => {
    var result = {};
  ...
  _.groupBy = group((result, key, value) => {
    (_.has(result, key) ? result[key] : (result[key] = [])).push(value);
  });

  // Indexes the object's values by a criterion, similar to `groupBy`, but for
  // when you know that your index values will be unique.
  _.indexBy = group((result, key, value) => {
    result[key] = value;
  });
  ...
  _.zip = function(...args) {
    var length = _.max(_.pluck(args, "length").concat(0));
  ...
  _.after = (times, func) => function(...args) {
    if (--times < 1) {
